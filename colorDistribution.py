...
def calculatePaletteDifference(percentages1, palette1, percentages2, palette2):
  sig1 = np.empty((len(percentages1), 4), dtype=np.float32)
  sig1[:, 0] = percentages1
  sig1[:, 1:] = palette1
  
  sig2 = np.empty((len(percentages2), 4), dtype=np.float32)
  sig2[:, 0] = percentages2
  sig2[:, 1:] = palette2
...
def calculateAllPaletteDifferences(percentages, palettes, names):
  ...
  for i in range(len(percentages)):
    if (i+1 == len(percentages)): break
    for j in range(i+1, len(percentages)):
      cost = calculatePaletteDifference(percentages[i], palettes[i], percentages[j], palettes[j])
...
def clustering(folderPath):
  ...
  plt.pie(x=percentages,labels=range(len(kmeans.cluster_centers_)), colors=centroidsToRgbPalette(ab_centers)/255.0)
  plt.plot()
  plt.show()
  

  return kmeans #ab_centers, percentages
...
if __name__ == "__main__":
  ...
  trainPalette, trainClusterPercentages = clustering("/home/leon/Documenti/TestTesi/datasets/nuovoDataset(0.0004,0.0004)/train")
  evalPalette, evalClusterPercentages = clustering("/home/leon/Documenti/TestTesi/datasets/nuovoDataset(0.0004,0.0004)/eval")
  testPalette, testClusterPercentages = clustering("/home/leon/Documenti/TestTesi/datasets/nuovoDataset(0.0004,0.0004)/test")
  ...
  res = calculateAllPaletteDifferences([trainClusterPercentages, evalClusterPercentages, testClusterPercentages], [trainPalette, evalPalette, testPalette], ["train", "eval", "test"])
  total = 0
  print(res)
  for cost in res:
    total+=cost["cost"]
  ...
  trainClusterPercentages, trainPalette = sortByPercentage(trainClusterPercentages, trainPalette)
  evalClusterPercentages, evalPalette = sortByPercentage(evalClusterPercentages, evalPalette)
  testClusterPercentages, testPalette = sortByPercentage(testClusterPercentages, testPalette)
  ...
  axs[0, 1].pie(trainClusterPercentages, colors=trainPalette, labels=list(map(rgbToHEX, trainPalette)))
  axs[0, 1].set_title("Train Palette")

  axs[1, 0].pie(evalClusterPercentages, colors=evalPalette, labels=list(map(rgbToHEX, evalPalette)))
  axs[1, 0].set_title("Eval Palette")

  axs[1, 1].pie(testClusterPercentages, colors=testPalette, labels=list(map(rgbToHEX, testPalette)))
